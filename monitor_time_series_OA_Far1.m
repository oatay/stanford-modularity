function [cell_data]=monitor_time_series_OA_Far1(flc,initial_tp,no_cell_of_interest,...
...
curr_plot_area=[all_obj.area(no_cell_of_interest,:) zeros(1,5)];
...
curr_plot_Far1nuc = (curr_plot_Far1nuc-autofl_nucfar1)./zdim;
curr_plot_Far1nuc(isnan(curr_plot_Far1nuc)) = 0;

curr_plot_Far1C = (0.5.*max(curr_plot_Whi5)./max(curr_plot_Far1C)).*curr_plot_Far1C;
curr_plot_Far1T = (0.4.*max(curr_plot_Whi5)./max(curr_plot_Far1T)).*curr_plot_Far1T;
curr_plot_Far1N = (0.5.*max(curr_plot_Whi5)./max(abs(curr_plot_Far1N))).*curr_plot_Far1N;
curr_plot_area = (0.2.*max(curr_plot_Whi5)./max(curr_plot_area)).*curr_plot_area;
curr_plot_Far1nuc = (1.*max(curr_plot_Whi5)./max(curr_plot_Far1nuc)).*curr_plot_Far1nuc;
curr_plot_Whi5;

%plot(curr_plot_area);pause
max_v=max(curr_plot_Whi5);

figure(1)
subplot(2,2,1)
plot(curr_plot_Far1nuc,'color',[0.0 0.5 0.0]);hold on;  % green
...
plot(curr_plot_area.^(3./2),'color',[0.7 0 0]); % volume; 3dark red
...
while checkbutton~=2
    ...
    if (button==29) %forward in time
        ...
        if current_time>no_tp
            ...
            figure(1)
            subplot(2,2,1);plot(curr_plot_Far1nuc,'color',[0.0 0.5 0.0]);hold on; plot(curr_plot_Far1C,'color',[0.0 0.2 1.0]);plot(curr_plot_Far1N,'color',[0.2 0.0 0.0]);
            plot(curr_plot_Whi5, 'color',[1.0 0.0 0.0]);plot(curr_plot_area.^(3./2),'color',[0.7 0 0]); title(['cell no = ' num2str(no_cell_of_interest)]);
            for ii=1:length(lim_h);plot([lim_h(ii) lim_h(ii)],[0 max_v],'k','linewidth',2); end
            plot(current_time,curr_plot_Far1nuc(current_time),'Marker','*','LineStyle','none','MarkerSize',20,'LineWidth',4,'Color',[0 0.5 0]);
...
if button ==1 || button ==3
    for j=1:3
        plot(curr_plot_Far1nuc,'color',[0.0 0.5 0.0]);hold on; plot(curr_plot_Far1C,'color',[0.0 0.2 1.0]);
...
%             cell_data(1,6)=min([numbM x1]);
        if j==1
            xlabel('Far1 (arrest start)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
            cell_data(1,5)=min([numbM x1]);
        elseif j==2
            xlabel('Far1 (arrest end)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
            cell_data(1,6)=min([numbM x1]);
        elseif j==3
            xlabel('Far1 (decay end)')
            [x1,y1,btmp]=ginput(1);
