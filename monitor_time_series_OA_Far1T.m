...
curr_plot_Far1N = [curr_plot_Far1nuc-autofl_nucfar1 zeros(1,5)];
...
curr_plot_Far1T = 100.*[curr_plot_Far1T(no_cell_of_interest,:)./volume zeros(1,5)];
...
curr_plot_Far1C=[all_obj.cyt_Far1(no_cell_of_interest,:) zeros(1,5)];
...
plot(curr_plot_area.^(3./2),'color',[0.7 0 0]); % dark red
...
%             cell_data(1,6)=min([numbM x1]);
        if j==1
            xlabel('Far1 nuclear exit (start)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
            cell_data(1,5)=min([numbM x1]);
        elseif j==2
            xlabel('Far1 nuclear exit (end)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
            cell_data(1,6)=min([numbM x1]);
        end
