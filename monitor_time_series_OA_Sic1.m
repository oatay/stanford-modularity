function [cell_data]=monitor_time_series_OA_Sic1(flc,initial_tp,no_cell_of_interest,...
    prefix,pos_num,suffix,type,suffix2,numbM,all_obj,lim_h)


%FAR1 == SIC1 for this program -- GFP channel
...
median(all_obj.med_backgr_Far1)
background1 = median(all_obj.med_backgr_Far1)-45+(74.3126+area(area < 3215).*0.0072);%%%background subt.
background3 = median(all_obj.med_backgr_Far1)-45+(100.0599-area(area >= 3215).*0.0011);
...
all_obj.nuc_Far1(all_obj.nuc_Far1 < 0) = 0;
all_obj.cyt_Far1(all_obj.cyt_Far1 < 0) = 0;
...
curr_plot_Far1N = [curr_plot_Far1nuc zeros(1,5)];

volume = [all_obj.volume(no_cell_of_interest,:)];

curr_plot_Far1T = all_obj.nuc_Far1.*all_obj.tot_nucl_areaR+all_obj.cyt_Far1.*all_obj.tot_cyt_areaR;
curr_plot_Far1T = curr_plot_Far1T(no_cell_of_interest,:);
curr_plot_Far1C = [curr_plot_Far1T./volume zeros(1,5)];
curr_plot_Far1C = (0.7.*max(curr_plot_Far1T)./max(curr_plot_Far1C)).*curr_plot_Far1C;
curr_plot_Far1T = [curr_plot_Far1T zeros(1,5)];
curr_plot_Whi5= (0.5.*max(curr_plot_Far1T)./max(curr_plot_Whi5)).*curr_plot_Whi5;
curr_plot_Far1N = (1.1.*max(curr_plot_Far1T)./max(curr_plot_Far1N)).*curr_plot_Far1N;
curr_plot_volume = [volume zeros(1,5)];
%plot(curr_plot_volume);pause
max_v=10+max(curr_plot_Far1T);

scale_factor3=max(curr_plot_Far1T)./max(curr_plot_volume);
curr_plot_volume=0.8.*scale_factor3.*curr_plot_volume;

figure(1)
subplot(2,2,1)
plot(curr_plot_Far1T,'color',[0.0 0.5 0.0]);hold on;  % green
plot(curr_plot_Far1C,'color',[0.0 0.2 1.0]); % blue
plot(curr_plot_Far1N,'color',[0.2 0.0 0.0]); % black
...
subplot(2,2,3)
imshow(outline+IN);title('Sic1-GFP')
text(10,10,['time = ' num2str(initial_tp)],'color',[0 1 0])
subplot(2,2,4)
...
%             cell_data(1,5)=min([numbM x1]);
        if j==1
            ...
            cell_data(1,5)=min([numbM x1]);
        elseif j==2
            xlabel('Sic1 nuclear exit (start)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
            cell_data(1,6)=min([numbM x1]);
        elseif j==3
            xlabel('Sic1 nuclear exit (end)')
            [x1,y1,btmp]=ginput(1);
            plot([x1 x1],[0 max_v],'color',[1 0 0],'linewidth',2)
...
% %         elseif j==5
% %             xlabel('mark initial point of Far1 concentration')
% %             [x1,y1,btmp]=ginput(1);
% %             plot([x1 x1],[0 max_v],'--','color',[1 0.0 1])
% %             cell_data(1,9)=x1;
% %         elseif j==6
% %             xlabel('mark end point of Far1 concentration')
% %             [x1,y1,btmp]=ginput(1);
% %             plot([x1 x1],[0 max_v],'--','color',[1 0.0 1])
% %             cell_data(1,10)=x1;
% %         end
